...
export default function DetailExperience({ data }) {
  ...
  return (<>
    ...
    <Bloom />
    <Nav forceSolid />
    <main className={`long-detail-page${data.type === 'Nexbash Studio' ? ' studio-detail-page' : ''}${data.type === 'Nexbash Service' ? ' service-detail-page' : ''}${data.type === 'Industry Solution' && data.gallery?.length ? ' industry-detail-page' : ''}`} id="top">
